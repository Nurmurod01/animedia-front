...
export default function BlogPostCard({ post, users }) {
  ...
  return (
    <Card className="overflow-hidden border-none anime-card">
          ...
          <Avatar className="h-8 w-8 border border-[#8a2be2]">
            {author.image ? (
              <Image
                ...
                height={50}
                width={50}
                alt={<AvatarFallback>{author.username.substring(0, 2).toUpperCase()}</AvatarFallback>}
                className="rounded-full"
              />
            ) : (
              <div>
                <AvatarImage src={author.image || "/placeholder.svg?height=32&width=32"} alt={author.username} />
